...
public class SpecularCubemapFilter implements Closeable {
  ...
  static {
    COORDS_BUFFER.put(new float[] {-1f, -1f, +1f, -1f, -1f, +1f, +1f, +1f,});
  }
  ...
  public void update(Image[] images) {
    try {
      ...
      for (int i = 0; i < NUMBER_OF_CUBE_FACES; ++i) {
        ...
        GLES30.glTexImage2D(GLES30.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GLES30.GL_RGBA16F, resolution, resolution, 0, GLES30.GL_RGBA, GLES30.GL_HALF_FLOAT, image.getPlanes()[0].getBuffer());
  ...
  private void initializeLdCubemap() {
    ...
    for (int level = 0; level < numberOfMipmapLevels; ++level) {
      ...
      for (int face = 0; face < NUMBER_OF_CUBE_FACES; ++face) {
        GLES30.glTexImage2D(GLES30.GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, level, GLES30.GL_RGB16F, mipmapResolution, mipmapResolution, 0, GLES30.GL_RGB, GLES30.GL_HALF_FLOAT, null);
  ...
  private Shader[] createShaders(SampleRender render, ChunkIterable chunks) throws IOException {
    ...
    for (Chunk chunk : chunks) {
      ...
      shaders[chunk.chunkIndex] =
          Shader.createFromAssets(render, "shaders/cubemap_filter.vert", "shaders/cubemap_filter.frag", defines)
